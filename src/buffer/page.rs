use crate::storage::{Block, LOCSIZ, Flags};
...
pub trait TupleCRUD {
    ...
    fn read(&self, tup_idx: u16, tup_siz: u16) -> Result<Option<Vec<u8>>, ()>;
    fn update(&mut self, tup_idx: u16, data: &[u8]) -> Option<()>;
    fn delete(&mut self, tup_idx: u16) -> Option<()>;
} 

impl TupleCRUD for Page {
    fn write(&mut self, data: &[u8]) -> Option<()> {
        let Some(block) = &mut self.block else {return None};
        let write_len = data.len() as u16;

        if block.lower + LOCSIZ >= block.upper - write_len {return None;}
    ...
    fn read(&self, tup_idx: u16, tup_siz: u16) -> Result<Option<Vec<u8>>, ()> {
        let Some(block) = &self.block else {return Err(())};
        let start = tup_idx * LOCSIZ;
        if start >= block.lower {return Err(())}
    ...
    fn update(&mut self, tup_idx: u16, data: &[u8]) -> Option<()> {
        let Some(block) = &mut self.block else {return None};
        let start = tup_idx * LOCSIZ;
        if start >= block.lower {return None;}
...
impl Page {
    ...
    pub fn add(&mut self, tuple: Tuple, schema: &Schema) -> Option<()> {
        if schema.len() != tuple.len() {return None;}
...
fn test_tuple_write() {
    ...
    p.write(&bincode::serialize(&tuple).unwrap());
...
fn test_tuple_update() {
    ...
    tuple = (11, 15);
    p.update(0, &bincode::serialize(&tuple).unwrap());
    let Some(b) = &p.block else {panic!()};
